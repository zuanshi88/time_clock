#refactor with a more developed class structure.
...
#@total time is going to keep track of the total about studied.
  ...
  def clock_out(in_and_out, time_in, total_time)
    ...
    new_log = "OUT: #{time.strftime("%m/%d")} @ #{time.strftime("%H:%M")}"
    ...
    time_in_for_calc = Time.parse(time_in)
      time_in_read.close
    session_elapsed_time = (time.to_f - time_in_for_calc.to_f)/3600

    total_time_read = File.open(total_time, "r")
    total_time_for_calc =  total_time_read.gets.to_i
  ...
  #stack_menu_files = [stack, count, total_time, in_and_out, time_in, log_book, idea_file]
#  stack_files = [stack, in_and_out, time_in, total_time, log_book, idea_file]
#files = [stack = "stack.txt", in_and_out = "in_and_out", time_in = "time_in", total_time = "total_time", log_book = "log_book", idea_file = "ideas.txt"]


  def stack_menu_selection(files)
    ...
    if selection == 0
      ...
      stack_menu_selection(stack_menu_files)
      ...
      #where we are in the line
        ...
        until count >= stack.length
          ...
          stack_menu_selection(stack_menu_files)        end
      elsif selection == 8
        main_menu(main_menu_files)
      else
        print_selection(file_array, count)
        stack_menu_selection(stack_menu_files)      end
    end
...
main_menu_files = [stack = "stack.txt", in_and_out = "in_and_out", time_in = "time_in", total_time = "total_time", log_book = "log_book", idea_file = "ideas.txt"]
...
__END__
05/09/2020: broke up log_book_3 to try and refactor it... running into
all sorts of problems... feeling the lack of sleep a little bit. Resenting that
I have to go into work right now!
